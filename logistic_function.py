...
SINE_MODEL_PATH_DIC = {
    'epinions': './2_特征处理/sine_epinions_models',
    'slashdot': './2_特征处理/sine_slashdot_models',
    'bitcoin_alpha': './2_特征处理/sine_bitcoin_alpha_models',
    'bitcoin_otc': './2_特征处理/sine_bitcoin_otc_models'
}

SIDE_MODEL_PATH_DIC = {
    'epinions': './2_特征处理/side_epinions_models',
    'slashdot': './2_特征处理/side_slashdot_models',
    'bitcoin_alpha': './2_特征处理/side_bitcoin_alpha_models',
    'bitcoin_otc': './2_特征处理/side_bitcoin_otc_models'
}
...
def read_train_test_data(dataset, k):
    train_X = []
    train_y = []
    with open('./experiment-data/{}-train-{}.csv'.format(dataset, k)) as f:
        for line in f:
            i, j, flag = line.split()
            i = int(i)
            j = int(j)
    ...
    with open('./experiment-data/{}-test-{}.csv'.format(dataset, k)) as f:
        for line in f:
            i, j, flag = line.split()
            i = int(i)
            j = int(j)
...
def logistic_embedding1(k=1, dataset='epinions'):
    """use deepwalk 2_特征处理 to train logistic function
    
    Returns:
        pos_ratio, accuracy, f1_score0, f1_score1, f1_score2, auc_score
    """

    fpath = os.path.join('2_特征处理/deepwalk_emb', '{}-{}.emb'.format(dataset, k))
...
def logistic_embedding2(k=1, dataset='epinions'):
    """use node2vec 2_特征处理 to train logistic function
    
    Returns:
        pos_ratio, accuracy, f1_score0, f1_score1, f1_score2, auc_score
    """

    fpath = os.path.join('2_特征处理/node2vec_emb', '{}-{}.emb'.format(dataset, k))
    embeddings = read_emb(fpath, dataset)
...
def logistic_embedding3(k=1, dataset='epinions'):
    """use line 2_特征处理 to train logistic function
    Returns:
        pos_ratio, accuracy, f1_score0, f1_score1, f1_score2, auc_score
    """
    fpath = os.path.join('2_特征处理/line_emb', '{}-{}.emb'.format(dataset, k))
    embeddings = read_emb(fpath, dataset)
...
def logistic_embedding5(k=1, dataset='epinions', epoch=50, v0=True):
    """use sine 2_特征处理 to train logistic function
    Returns:
        pos_ratio, accuracy, f1_score0, f1_score1, f1_score2, auc_score
    """
...
def logistic_embedding6(k=1, dataset='epinions', epoch=1):
    """use side 2_特征处理 to train logistic function
    Returns:
        pos_ratio, accuracy, f1_score0, f1_score1, f1_score2, auc_score
    """
    def read_side_emb():
        voc_path = os.path.join('2_特征处理/side', '{}{}.vocab'.format(dataset, k))
        ...
        embed_path = os.path.join('2_特征处理/side', '{}{}{}.emb'.format(dataset, k, epoch))
...
def logistic_embedding7(k=1, dataset='epinions', dirname="sign2vec"):
    """use signet 2_特征处理 to train logistic function
    Returns:
        pos_ratio, accuracy, f1_score0, f1_score1, f1_score2, auc_score
    """
    print('signet', k, dataset)
    filename = os.path.join('2_特征处理', dirname, '2_特征处理-{}-{}.npy'.format(dataset, k))
    embeddings = np.load(filename)
...
def logistic_embedding9(k=1, dataset='epinions', epoch=10, dirname='sigat'):
    ...
    filename = os.path.join('2_特征处理', dirname, 'embedding-{}-{}-{}.npy'.format(dataset, k, epoch))
    embeddings = np.load(filename)
