...
class DSConvdisease(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, kernel_size, extend_scope, morph,
        ...
        """
        super(DSConvdisease, self).__init__()
        ...
        self.gn = nn.GroupNorm(out_ch // 2, out_ch)
...
if __name__ == '__main__':
    ...
    # A = np.ones(shape = (1, 3, 64, 64), dtype = np.float32)
    A = np.random.rand(1, 64, 64, 1)

    # 数组转换
    reshaped_array = np.squeeze(A)
    # 打印转换后的矩阵表示
    ...
    # 数组转换
    reshaped_array1 = np.squeeze(out)
    # 打印转换后的矩阵表示
